...
import cn.enaium.joe.core.task.SearchFieldTask
 ...
 * @since 2.0.0
 */
class SearchFieldDialog : Dialog(i18n("search.field.title")) {
    init {
        content = BorderPane().apply {
            ...
            top = MigPane("fillx", "[fill][fill]").apply {
                ...
                add(Button(i18n("menu.search")).apply {
                    setOnAction {
                        if (owner.text.isBlank() && name.text.isBlank() && description.text.isBlank()) {
                            warning("Please input!")
                            return@setOnAction
                        }
                        searchResultList.items.clear()
                        task.submit(SearchFieldTask(jar, owner.text, name.text, description.text)).thenAccept {
                            it.forEach { result ->
